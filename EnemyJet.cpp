...
EnemyJet::EnemyJet(Game* r_pGame, point ref, double s, color fc, color bc) :enemy(r_pGame, ref, s, fc, bc)
{
	width = 660 * s;
	hight = 240 * s;
	stype = "EnemyJet";
...
void EnemyJet::collisionAction(GameObject* other)
{
	if (other->get_type() == "Bullet") {
		pGame->getstatusBar()->increment_status(3, 100);
	}
...
void EnemyJet::save(string f)
{
	ofstream OutFile;
	OutFile.open(f);
	OutFile << "enemyjet " << RefPoint.x << " " << RefPoint.y << " " << s << endl;
	OutFile.close();
}

void EnemyJet::load(string f)
{
	ifstream Infile;
	Infile.open(f);
	bool label;
